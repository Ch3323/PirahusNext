...
function TimerBar({ seconds, max }: { seconds: number; max: number }) {
  ...
  return (
    <div style={{ width: "100%", height: "3px", background: "#1f2937", borderRadius: "2px" }}>
      <div style={{
        width: `${pct * 100}%`, height: "100%", background: color, borderRadius: "2px",
        transition: "width 0.5s linear, background 0.5s", boxShadow: `0 0 6px ${color}`,
      }} />
    </div>
  );
}
...
function CodeBlock({ code }: { code: string }) {
  return (
    <div style={{ background: "rgba(0,0,0,0.6)", border: "1px solid #374151", borderRadius: "0.25rem", padding: "1rem 1.25rem", width: "100%" }}>
      {code.split("\n").map((line, i) => (
  ...
  return (
    <div style={{ position: "relative", minHeight: "100vh", overflow: "hidden", backgroundColor: "#000", ...pixelifySans.style }}>
      ...
      <div style={{ position: "relative", zIndex: 1, minHeight: "100vh" }}>
        ...
        {/* Content */}
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: "5rem 1rem 2rem", gap: "1.5rem" }}>
          ...
          {phase === "playing" && currentQ && (
            <div style={{ width: "100%", maxWidth: "600px", display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <div style={{ color: "#6b7280", fontSize: "0.75rem", letterSpacing: "0.15em" }}>WHAT DOES THIS PRINT?</div>
              ...
              {currentQ.type === "type" ? (
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  <input
                    ...
                    style={{
                      flex: 1, background: "rgba(15,15,20,0.88)",
                      border: `1px solid ${feedback === "correct" ? "#4ade80" : feedback === "wrong" ? "#f87171" : "#374151"}`,
                      color: "#d1d5db", fontFamily: shareTechMono.style.fontFamily,
                      fontSize: "1rem", padding: "0.5rem 0.75rem", borderRadius: "0.25rem",
                  ...
                  {currentQ.choices?.map((choice) => {
                    ...
                    return (
                      <button key={choice} onClick={() => handleChoiceClick(choice)} disabled={!!feedback} style={{
                        ...
                        fontFamily: shareTechMono.style.fontFamily, fontSize: "1.1rem",
                        padding: "0.75rem", borderRadius: "0.25rem",
                        cursor: feedback ? "default" : "pointer", transition: "all 0.2s",
                        letterSpacing: "0.1em", textShadow: showResult && isCorrect ? "0 0 8px #4ade80" : "none",
                      }}>
                        {choice}
                      </button>
                    );
              ...
              {feedback && (
                <div style={{ color: feedback === "correct" ? "#4ade80" : "#f87171", fontSize: "0.8rem", fontFamily: shareTechMono.style.fontFamily, letterSpacing: "0.15em", textAlign: "center" }}>
                  {feedback === "correct" ? `✓ CORRECT  +${currentQ.timeBonus}s` : `✗ WRONG · answer was ${currentQ.answer}`}
                </div>
              )}
            </div>
          )}
